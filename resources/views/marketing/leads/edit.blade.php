...
@section('content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi lead</p>
        </div>
        ...
        <form action="{{ route('marketing.leads.update', $lead) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <select name="client_id"
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" @selected($lead->client_id == $client->id)>
                            {{ $client->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <textarea name="notes" rows="3"
                          ...
                          placeholder="Catatan tambahan">{{ $lead->notes }}</textarea>
                @error('notes')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
