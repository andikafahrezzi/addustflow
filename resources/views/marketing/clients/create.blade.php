...
@section('content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Tambah Client</h1>
            <p class="text-sm text-gray-500 mt-1">Isi form berikut untuk menambahkan client baru</p>
        </div>
        ...
        <form action="{{ route('marketing.clients.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="address" rows="3"
                          ...
                          placeholder="Alamat lengkap client">{{ old('address') }}</textarea>
                @error('address')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
